...
#include <Frostium3D/Libraries/stb_image/stb_image.h>
#include <fstream>
#include <algorithm>
#include <sys/stat.h>
...
namespace ifd {
	...
	bool FileDialog::IsDone(const std::string& key)
	{
		...
		if (isMe && m_isOpen)
		{
			...
			ImGui::Begin(m_currentTitle.c_str(), &m_isOpen, ImGuiWindowFlags_NoScrollbar);
			m_renderFileDialog();
			ImGui::End();
		}
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				...
				if (ImGui::BeginDragDropSource())
				{
					...
					ImGui::SetDragDropPayload("FileBrowser", &entry.Path, sizeof(std::filesystem::path));
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		/***** CONTENT *****/
		float bottomBarHeight = (GImGui->FontSize + ImGui::GetStyle().FramePadding.y * 2 + ImGui::GetStyle().ItemSpacing.y * 2.0f) / 6.0;
