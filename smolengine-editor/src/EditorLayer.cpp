...
namespace SmolEngine
{
	...
	void EditorLayer::OnEndFrame(DeltaTime deltaTime)
	{
		...
		if (m_World->IsInPlayMode() == false)
		{
			...
			RendererSystem::DebugDraw(&state);
		}
	}
	...
	void EditorLayer::DrawRendererStateComponent(RendererStateComponent* component)
	{
		RendererState& render_state = component->State;
		const float padding = 130.0f;
		ImGui::SetWindowFontScale(0.8f);
		...
		if (ImGui::TreeNodeEx("Environment"))
		{
			...
			ImGui::Extensions::Combo("Type", "Dynamic\0Static\0", component->EnvironmentFlags, padding, "IBL", 12.0f);
			if (component->EnvironmentFlags == 0)
			{
				...
				ImGui::Extensions::InputFloat("Sun Intensity", component->SkyProperties.SunIntensity, padding, "IBL", 12.0f);
				ImGui::Extensions::InputFloat("Planet Radius", component->SkyProperties.PlanetRadius, padding, "IBL", 12.0f);
				ImGui::Extensions::InputFloat("Atm Radius", component->SkyProperties.AtmosphereRadius, padding, "IBL", 12.0f);
				ImGui::Extensions::InputFloat("Mie Direction", component->SkyProperties.MieScatteringDirection, padding, "IBL", 12.0f);
				...
				ImGui::Extensions::CheckBox("Environment Map", component->bGeneratePBRMaps, padding, "IBL", 12.0f);

				ImGui::NewLine();
				ImGui::SetCursorPosX(12.0f);
				if (ImGui::Button("Update", { ImGui::GetWindowWidth() - 20.0f, 30.0f }))
				{
					DeferredRenderer::SetDynamicSkyboxProperties(component->SkyProperties, component->bGeneratePBRMaps);
				}
				...
				if (ImGui::Button("Reset", { ImGui::GetWindowWidth() - 20.0f, 30.0f }))
				{
					...
					DeferredRenderer::SetDynamicSkyboxProperties(component->SkyProperties, true);
				}
					...
					if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("FileBrowser"))
					{
						...
						if (EditorLayer::FileExtensionCheck(path, ".ktx"))
						{
							...
							if (texCI.Load(path) == true)
							{
								...
								DeferredRenderer::SetEnvironmentCube(component->CubeMap);
							}
						}
					}
					ImGui::EndDragDropTarget();
			...
			{
				DeferredRenderer::SetRendererState(&component->State);
			}
			...
			{
				glm::vec3* dir = (glm::vec3*) &component->DirLight.Direction;
				bool* is_active = (bool*)&component->DirLight.IsActive;

				if (ImGui::Extensions::CheckBox("Directional Light", *is_active, padding, "IBL", 12.0f))
				{
					DeferredRenderer::SubmitDirLight(&component->DirLight);
				}

				if (ImGui::Extensions::Combo("Shadows", "None\0Hard\0Soft\0", component->ShadowsFlags, padding, "IBL", 12.0f))
				{
					RendererStateComponent::ShadowType type = (RendererStateComponent::ShadowType)component->ShadowsFlags;
					switch (type)
					{
					case RendererStateComponent::ShadowType::None: component->DirLight.IsCastShadows = false; break;
					case RendererStateComponent::ShadowType::Hard: component->DirLight.IsCastShadows = true; component->DirLight.IsUseSoftShadows = false; break;
					case RendererStateComponent::ShadowType::Soft: component->DirLight.IsCastShadows = true; component->DirLight.IsUseSoftShadows = true; break;
					}

					DeferredRenderer::SubmitDirLight(&component->DirLight);
				}

				if (ImGui::Extensions::InputFloat("Intensity", component->DirLight.Intensity, padding, "IBL", 12.0f))
				{
					DeferredRenderer::SubmitDirLight(&component->DirLight);
				}
				...
				if (ImGui::Extensions::ColorInput4("Color", component->DirLight.Color, padding, "IBL", 12.0f))
				{
					DeferredRenderer::SubmitDirLight(&component->DirLight);
				}
			}
		...
		if (ImGui::TreeNodeEx("FXAA Settings"))
		{
			ImGui::NewLine();
		...
		if (ImGui::TreeNodeEx("Volumes"))
		{
			ImGui::NewLine();
			ImGui::SetCursorPosX(12.0f);
			ImGui::TextUnformatted("Dirt Mask");
			auto& dirt = component->DirtMask;
			if (dirt.bReset)
			{
				dirt = {};
	...
	void EditorLayer::DrawComponents()
	{
		...
		for (uint32_t i = 0; i < m_SelectedActor->GetComponentsCount(); ++i)
		{
			...
			if (IsCurrentComponent<RendererStateComponent>(i))
			{
				if (ImGui::CollapsingHeader("Renderer State"))
				{
					auto component = m_World->GetActiveScene()->GetComponent<RendererStateComponent>(m_SelectedActor);
					DrawRendererStateComponent(component);
				}
			}
	...
	void EditorLayer::DrawComponentPopUp()
	{
		if (ImGui::BeginPopup("AddComponentPopUp"))
		{
			...
			ImGui::MenuItem("#Systems", NULL, false, false);
			{
				ImGui::Separator();

				if (ImGui::MenuItem("Renderer State"))
				{
					m_World->GetActiveScene()->AddComponent<RendererStateComponent>(m_SelectedActor);
					ImGui::CloseCurrentPopup();
				}
		...
		if (scene->HasComponent<ScriptComponent>(m_SelectedActor))
		{
			...
			for (auto& script : comp->CppScripts)
			{
				...
				if (ImGui::CollapsingHeader(title.c_str()))
				{
					...
					{
						if (field.type == FiledDataFlags::Int32)
						{
							ImGui::Extensions::InputInt(field.name, *(int32_t*)field.ptr);
						}

						if (field.type == FiledDataFlags::Float)
						{
							ImGui::Extensions::InputFloat(field.name, *(float*)field.ptr);
						}
