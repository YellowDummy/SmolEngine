...
namespace SmolEngine
{
	...
	int32_t MaterialLibrary::Add(MaterialCreateInfo* infoCI, const std::string& path)
	{
		...
		Material newMaterial = {};
		{
			...
			{
				JobsSystem::Schedule([&]() {Texture::LoadTexture(infoCI->Textures[MaterialTexture::Albedro], &albedroData); });
				JobsSystem::Schedule([&]() {Texture::LoadTexture(infoCI->Textures[MaterialTexture::Normal], &normalData); });
				JobsSystem::Schedule([&]() {Texture::LoadTexture(infoCI->Textures[MaterialTexture::Metallic], &metallicData); });
				JobsSystem::Schedule([&]() {Texture::LoadTexture(infoCI->Textures[MaterialTexture::Roughness], &roughnessData); });
				JobsSystem::Schedule([&]() {Texture::LoadTexture(infoCI->Textures[MaterialTexture::AO], &aoData); });
			}
